...
export default function CallbackClient() {
  ...
  useEffect(() => {
    ...
    const bridgeAuth = async () => {
      try {
        // Step 1: Exchange the session verifier for a cookie via the Neon Auth client.
        // This handles the ?neon_auth_session_verifier= param automatically.
        await authClient.getSession();

        // Step 2: Call the Next.js bridge route which reads the now-set session
        // cookie, verifies it server-side, then calls the backend to sync the
        // user and create a local JWT.
        const response = await fetch("/api/auth/bridge-session");

        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
          throw new Error(
            payload?.message || `Bridge returned ${response.status}`,
          );
        }
        ...
        console.error("[social-callback] Bridge auth failed:", err);
        setError(
