...
export default function AssessmentDetailPage() {
  ...
  const handleStart = async () => {
    ...
    setStarting(true);
    try {
      const payload = await apiFetch<StartAttemptPayload>(`/api/assessments/${assessmentId}/start`, {
        method: "POST",
      });
      window.sessionStorage.setItem(`beelearn-attempt:${payload.attemptId}`, JSON.stringify(payload));
      router.push(`/assessments/${assessmentId}/take?attemptId=${encodeURIComponent(payload.attemptId)}`);
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Unable to start assessment.");
    } finally {
      setStarting(false);
    }
  };
          ...
          <CardContent>
            <Stack spacing={2}>
              <Stack direction="row" spacing={1} flexWrap="wrap" alignItems="center">
                <Chip size="small" variant="outlined" label={formatType(data.assessment.type)} />
                {data.assessment.grade ? (
                  <Chip size="small" variant="outlined" label={`Grade ${data.assessment.grade}`} />
                ) : null}
                {data.assessment.timeLimitMinutes ? (
                  <Chip size="small" color="primary" label={`${data.assessment.timeLimitMinutes} min`} />
                ) : (
                  <Chip size="small" color="primary" label="Untimed" />
                )}
                <Chip size="small" variant="outlined" label={`${questionCount} questions`} />
              </Stack>
              ...
              {data.assessment.instructions ? (
                <>
                  <Divider />
                  <Typography variant="body2">{data.assessment.instructions}</Typography>
                </>
              ) : null}
